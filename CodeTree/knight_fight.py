...
"""
Solution.
1.초기 변수를 제대로 선언하는 것이 가장 중요한 문제
1-1.각 기사의 (r, c, h, w, k, bef_k, nr, nc, dmg, is_moved) 데이터를 각각 기사의 id에 맞게 분리해서 저장해야 함
    만약, 분리하지 않으면 기사끼리 충돌이 일어났을 때와 일어나지 않았을 때 현재 기사의 정보를 업데이트 하기 매우 어려움
2.조건 수행 문은 크게 세 가지로 구분됨
2-1.선택한 기사가 움직였을 떄, 범위를 벗어나는지 판별
2-2.범위를 벗어나지 않는다면, 함정 또는 벽과 충돌하는지 판별
2-3.선택한 기사의 범위에 다른 기사가 있는지 체크하고 있다면 밀려난 기사를 q에 대입하여 반복
3. 이때, movement() 함수 호출에 사용된 idx(=현재 기사)의 데미지는 while 문이 종료될 때 0으로 초기화 해야 한다. (중요!)
"""
